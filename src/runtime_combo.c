...
#define DT_DRV_COMPAT zmk_runtime_combos
...
// Reconstruct a zmk_behavior_binding from rt_params for the given combo index.
static int make_binding(int combo_idx, struct zmk_behavior_binding *out) {
    // local_id 0 == unmodified DT default. In settings-table local-id mode the
    // DT binding carries a valid behavior_dev (name) but local_id 0, so use the
    // const binding directly; reconstruction from local_id is only for runtime-
    // set bindings (non-zero live local_id supplied by the host).
    if (rt_params[combo_idx].behavior_local_id == 0) {
        *out = combos[combo_idx].behavior;
        return 0;
    }
...
static bool combo_active_on_layer(const struct combo_cfg *combo, uint8_t layer) {
    if (!rt_params[combo - combos].layer_mask) {
        return true;
    }

    return rt_params[combo - combos].layer_mask & BIT(layer);
}

static bool is_quick_tap(const struct combo_cfg *combo, int64_t timestamp) {
    return (last_tapped_timestamp + rt_params[combo - combos].require_prior_idle_ms) > timestamp;
}
...
static int64_t first_candidate_timeout() {
    ...
    for (int i = 0; i < ARRAY_SIZE(combos); i++) {
        if (sys_bitfield_test_bit((mem_addr_t)&candidates, i)) {
            first_timeout = MIN(first_timeout, rt_params[i].timeout_ms);
        }
    }
...
static int filter_timed_out_candidates(int64_t timestamp) {
    ...
    for (int i = 0; i < ARRAY_SIZE(combos); i++) {
        if (sys_bitfield_test_bit((mem_addr_t)&candidates, i)) {
            ...
            if (pressed_keys[0].data.timestamp + rt_params[i].timeout_ms > timestamp) {
...
const struct zmk_listener zmk_listener_runtime_combo;
...
static inline int press_combo_behavior(int combo_idx, const struct combo_cfg *combo,
    ...
    int rc = make_binding(combo_idx, &binding);
    if (rc) {
        return rc;
    }
...
static bool release_combo_key(int32_t position, int64_t timestamp) {
    for (int combo_idx = 0; combo_idx < active_combo_count; combo_idx++) {
        ...
        if (key_released) {
            ...
            if ((rt_params[c - combos].slow_release && all_keys_released) ||
                (!rt_params[c - combos].slow_release && all_keys_pressed)) {
...
ZMK_LISTENER(runtime_combo, behavior_combo_listener);
