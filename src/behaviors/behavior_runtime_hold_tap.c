...
#define DT_DRV_COMPAT zmk_behavior_runtime_hold_tap
...
struct behavior_runtime_hold_tap_config {
    int tapping_term_ms;
...
struct behavior_runtime_hold_tap_data {
    ...
    // Runtime-editable timing (seeded from devicetree at init, then overridden
    // from NVS). Read on every keypress by the decision logic below.
    uint8_t slot;
...
struct active_hold_tap {
    ...
    int64_t timestamp;
    enum status status;
    const struct behavior_runtime_hold_tap_config *config;
    struct behavior_runtime_hold_tap_data *data;  // runtime-editable timing (mutable)
...
static bool is_quick_tap(struct active_hold_tap *hold_tap) {
    if ((last_tapped.timestamp + hold_tap->data->t.require_prior_idle_ms) > hold_tap->timestamp) {
        return true;
    } else {
        return (last_tapped.position == hold_tap->position) &&
               (last_tapped.timestamp + hold_tap->data->t.quick_tap_ms) > hold_tap->timestamp;
    }
}
...
const struct zmk_listener zmk_listener_behavior_runtime_hold_tap;
...
static void release_captured_events() {
    ...
    for (int i = 0; i < ZMK_BHV_HOLD_TAP_MAX_CAPTURED_EVENTS; i++) {
        ...
        case ET_CODE_CHANGED:
            ...
            ZMK_EVENT_RAISE_AT(captured_event->data.keycode, behavior_runtime_hold_tap);
            break;
        case ET_POS_CHANGED:
            ...
            ZMK_EVENT_RAISE_AT(captured_event->data.position, behavior_runtime_hold_tap);
            break;
        default:
...
static void decide_hold_tap(struct active_hold_tap *hold_tap,
    ...
    LOG_DBG("%d decided %s (%s decision moment %s)", hold_tap->position,
            status_str(hold_tap->status), flavor_str(hold_tap->data->t.flavor),
...
static int on_hold_tap_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    const struct behavior_runtime_hold_tap_config *cfg = dev->config;
    ...
    // Cache the mutable per-instance timing so the decision logic reads the
    // runtime values (NVS/DT) instead of the const config.
    hold_tap->data = dev->data;
    ...
    int32_t tapping_term_ms_left =
        (hold_tap->timestamp + hold_tap->data->t.tapping_term_ms) - k_uptime_get();
...
static int on_hold_tap_binding_released(struct zmk_behavior_binding *binding,
    ...
    if (event.timestamp > (hold_tap->timestamp + hold_tap->data->t.tapping_term_ms)) {
...
static int hold_tap_parameter_metadata(const struct device *hold_tap,
    ...
    const struct behavior_runtime_hold_tap_config *cfg = hold_tap->config;
    struct behavior_runtime_hold_tap_data *data = hold_tap->data;
    int err;
...
static const struct behavior_driver_api behavior_runtime_hold_tap_driver_api = {
...
static int position_state_changed_listener(const zmk_event_t *eh) {
    ...
    // have run out.
    if (ev->timestamp >
        (undecided_hold_tap->timestamp + undecided_hold_tap->data->t.tapping_term_ms)) {
...
int behavior_runtime_hold_tap_listener(const zmk_event_t *eh) {
...
ZMK_LISTENER(behavior_runtime_hold_tap, behavior_runtime_hold_tap_listener);
ZMK_SUBSCRIPTION(behavior_runtime_hold_tap, zmk_position_state_changed);
// this should be modifiers_state_changed, but unfrotunately that's not implemented yet.
ZMK_SUBSCRIPTION(behavior_runtime_hold_tap, zmk_keycode_state_changed);

void behavior_runtime_hold_tap_timer_work_handler(struct k_work *item) {
...
static int behavior_runtime_hold_tap_init(const struct device *dev) {
    static bool init_first_run = true;

    if (init_first_run) {
        for (int i = 0; i < ZMK_BHV_HOLD_TAP_MAX_HELD; i++) {
            k_work_init_delayable(&active_hold_taps[i].work, behavior_runtime_hold_tap_timer_work_handler);
            active_hold_taps[i].position = ZMK_BHV_HOLD_TAP_POSITION_NOT_USED;
        }
    }
    init_first_run = false;

    // Seed this instance's runtime timing from its devicetree defaults, then
    // register with the store so any NVS-saved value overrides it (now if
    // settings already loaded, or via the settings set-callback otherwise).
    // The const config is kept as the source of DT defaults for reset.
    struct behavior_runtime_hold_tap_data *data = dev->data;
    const struct behavior_runtime_hold_tap_config *cfg = dev->config;
    struct rt_holdtap_timing dt = {
...
#define KP_INST(n)                                                                                 \
    ...
    static const struct behavior_runtime_hold_tap_config behavior_runtime_hold_tap_config_##n = {                  \
    ...
    static struct behavior_runtime_hold_tap_data behavior_runtime_hold_tap_data_##n = {.slot = n};                 \
    BEHAVIOR_DT_INST_DEFINE(n, behavior_runtime_hold_tap_init, NULL, &behavior_runtime_hold_tap_data_##n,          \
                            &behavior_runtime_hold_tap_config_##n, POST_KERNEL,                            \
                            CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &behavior_runtime_hold_tap_driver_api);
